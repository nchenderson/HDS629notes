...
* The **missing at random** (MAR) assumption implies we only need to work with the distribution $\mathbf{Z}_{mis}|\mathbf{Z}_{obs}$. 
   + The MAR assumption implies the distribution of $\mathbf{Z}_{mis}|\mathbf{Z}_{obs}, \mathbf{R}$ is the same as $\mathbf{Z}_{mis}|\mathbf{Z}_{obs}$
...
* With multiple imputation, $\hat{\theta}$ is trying to find a solution of the following
estimating equation
\begin{equation}
\frac{1}{K} \sum_{k=1}^{K} E\Big\{ \theta \Big| \mathbf{Z}_{obs}, \mathbf{R}, \mathbf{Z}_{mis}^{(k)} \Big\} \approx E\Big\{ U(\theta; \mathbf{Z}_{mis}, \mathbf{Z}_{obs}) | \mathbf{Z}_{obs}, \mathbf{R} \Big} = 0
\end{equation}
