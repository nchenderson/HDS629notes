...
```{r, echo=FALSE, message=FALSE, fig.cap="Plots of individual-specific trajectories from the sleepstudy data with the trajectories of 4 individuals highlighted.", cache=TRUE}
library(lme4)
data(sleepstudy)
...
```{r, echo=FALSE, fig.cap="Separately estimated regression lines for 3 subjects in the sleepstudy data.", cache=TRUE}
pc <- c(1,4,3,4,5,6,16)
...
```{r, echo=TRUE, cache=TRUE, fig.cap="Random trajectories for sleepstudy data using the estimated intercept and slope random-effects variances."}
...
```{r, echo=TRUE, fig.cap="Distribution of Wheezing probability across individuals for different values of age and smoking status", cache=TRUE}
