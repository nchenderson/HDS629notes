...
```{r, echo=FALSE}
fitted_vals <- predict(proper_mod, newdat=newdataset)
oo <- order(xx_new)
yl0 <- min(ConformalInterval[,1])
yl1 <- max(ConformalInterval[,2])
plot(xx_new[oo], fitted_vals[oo], xlab="xx", ylab="predicted values", main="Fitted values and conformal prediction intervals",
     ylim=c(yl0, yl1))
lines(xx_new[oo], ConformalInterval[oo,1], lwd=2, lty=3)
lines(xx_new[oo], ConformalInterval[oo,2], lwd=2, lty=3)
```
